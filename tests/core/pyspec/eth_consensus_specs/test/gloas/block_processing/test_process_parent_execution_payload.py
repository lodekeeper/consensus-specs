...
def _setup_parent_payload_state(
    ...
    if value is None:
        value = spec.Gwei(0)
    if execution_requests_root is None:
...
def test_process_parent_execution_payload_full_parent(spec, phases):
    ...
    # Update parent bid's execution_requests_root to match the requests
    state.latest_execution_payload_bid.execution_requests_root = execution_requests.hash_tree_root()
