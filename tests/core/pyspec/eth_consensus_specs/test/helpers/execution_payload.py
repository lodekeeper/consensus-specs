...
def get_execution_payload_bid(spec, state, execution_payload):
    ...
    return spec.ExecutionPayloadBid(
        ...
        execution_requests_root=hash_tree_root(spec.ExecutionRequests()),
    )
...
def build_empty_post_gloas_execution_payload_bid(spec, state):
    ...
    # Use a deterministic synthetic hash derived from the current slot and parent hash.
    # This keeps helper-built empty payload bids distinct across consecutive empty blocks,
    # which is necessary for FULL vs EMPTY parent-path tests around fork transitions.
    empty_payload_hash = spec.Hash32(
        sha256(state.latest_block_hash + int(state.slot).to_bytes(8, "little")).digest()
    )
...
def build_signed_execution_payload_envelope(spec, state, block_root, signed_block):
    ...
    # Note: process_execution_payload no longer mutates state, so no state simulation needed.
    # state_root was removed from ExecutionPayloadEnvelope in deferred-payload redesign.
